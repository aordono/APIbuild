    ...
    for (i = 0; i < tattooStudios.length; i++) {  
        ...
        google.maps.event.addListener(marker, 'click', (function selectStudio(marker, i) {
            return function() {
                ...
                switch (tattooStudios[i]) {
                  case tattooStudios[0]:
                    skintricate.run();
                    console.log("skintricate");
                    break;
                  case tattooStudios[1]:
                    lighthouse.run();
                    console.log("lighthouse");
                    break;
                  case tattooStudios[2]:
                    fyink.run();
                    console.log("fyink");
                    break;
                  case tattooStudios[3]:
                    gastown.run();
                    console.log("gastown");
                    break;
                  case tattooStudios[4]:
                    atomiczombie.run();
                    console.log("atomic zombie");
                    break;
                  default:
                    //Statements executed when none of the values match the value of the expression
                    console.log("switch studios");
                    break;
                }
            }
        })(marker, i));
